...
typedef enum : NSUInteger {
    SGAlertViewBottomViewTypeOne,  // 确定 一个按钮
    SGAlertViewBottomViewTypeTwo,   // 取消  确定  两个按钮
} SGAlertViewBottomViewType;

@protocol SGAlertViewDelegate <NSObject>

@optional
/** right 【确定】按钮的点击事件(SGAlertViewBottomViewTypeOne 类型的确定按钮；SGAlertViewBottomViewTypeTwo 类型是right) */
- (void)didSelectedRightButtonClick;
/** left 【取消】按钮的点击事件(点击事件针对 SGAlertViewBottomViewTypeTwo 类型起作用) */
